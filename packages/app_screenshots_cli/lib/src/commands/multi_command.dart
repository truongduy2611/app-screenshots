...
class _MultiOpenCommand extends Command<int> {
  @override String get name => 'open';
  @override String get description => 'Open the multi-screenshot editor for a device type';
...
class _MultiStateCommand extends Command<int> {
  ...
  @override String get description => 'Get multi-editor state with all designs';
...
class _MultiSwitchCommand extends Command<int> {
  @override String get name => 'switch';
  @override String get description => 'Switch active design by index';

  _MultiSwitchCommand() {
    argParser.addOption('index', abbr: 'i', help: 'Design index (0-based)', mandatory: true);
  }
  ...
  Future<int> run() async {
    ...
    final client = await AppClient.discover();
    final result = await client.post(MultiAction.switchDesign.path, {'index': index});
    Output.print(result, json: Output.isJson(globalResults));
    client.close();
    return result['ok'] == true ? 0 : 1;
  }
...
class _MultiAddCommand extends Command<int> {
  ...
  @override String get description => 'Add a new design slot';
...
class _MultiRemoveCommand extends Command<int> {
  ...
  @override String get description => 'Remove a design (default: active)';
...
class _MultiDuplicateCommand extends Command<int> {
  @override String get name => 'duplicate';
  @override String get description => 'Duplicate a design';
...
class _MultiReorderCommand extends Command<int> {
  @override String get name => 'reorder';
  @override String get description => 'Reorder designs';
  ...
  Future<int> run() async {
    ...
    final client = await AppClient.discover();
    final result = await client.post(MultiAction.reorder.path, {'from': from, 'to': to});
    Output.print(result, json: Output.isJson(globalResults));
    client.close();
    return result['ok'] == true ? 0 : 1;
  }
...
class _MultiApplyPresetCommand extends Command<int> {
  ...
  @override String get description => 'Apply preset to all designs';
  ...
  Future<int> run() async {
    final client = await AppClient.discover();
    final result = await client.post(MultiAction.applyPreset.path, {'id': argResults!['id']});
    Output.print(result, json: Output.isJson(globalResults));
    client.close();
    return result['ok'] == true ? 0 : 1;
  }
...
class _MultiBatchCommand extends Command<int> {
  ...
  @override String get description => 'Apply same change to all designs';

  _MultiBatchCommand() {
    argParser.addOption('action', abbr: 'a', help: 'Batch action (set-background, set-padding, set-corner-radius)', mandatory: true);
    argParser.addOption('color', help: 'Color for set-background (hex)');
    argParser.addOption('value', abbr: 'v', help: 'Value for set-padding or set-corner-radius');
  }
  ...
  Future<int> run() async {
    ...
    if (value != null) {
      body[action == 'set-padding' ? 'padding' : 'radius'] = double.parse(value);
    }
    final client = await AppClient.discover();
...
class _MultiSetImageCommand extends Command<int> {
  ...
  @override String get description => 'Upload/set image for a specific design slot';

  _MultiSetImageCommand() {
    argParser.addOption('file', abbr: 'f', help: 'Path to image file', mandatory: true);
    argParser.addOption('index', abbr: 'i', help: 'Design index (default: active)');
  }
...
class _MultiSaveDesignCommand extends Command<int> {
  ...
  @override String get description => 'Save multi-design project to library';

  _MultiSaveDesignCommand() {
    argParser.addOption('name', abbr: 'n', help: 'Project name');
    argParser.addFlag('override', help: 'Override existing saved design instead of creating a new one', defaultsTo: false);
  }
