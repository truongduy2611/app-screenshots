  ...
  Future<Map<String, dynamic>> handleLibrary(
    ...
    switch (libraryAction) {
      case LibraryAction.list:
        ...
        return ServerResponse.ok(designs.map((d) => {
          'id': d.id,
          'name': d.name,
      ...
      case LibraryAction.folders:
        ...
        return ServerResponse.ok(folders.map((f) => {
          'id': f.id,
          'name': f.name,
      ...
      case LibraryAction.get:
        ...
        if (design == null) return ServerResponse.error('Design not found: $id');
      ...
      case LibraryAction.createFolder:
        ...
        final folder = await _persistenceService.createFolder(name, parentId: parentId);
        _libraryCubit?.loadDesigns();
      ...
      case LibraryAction.import_:
        ...
        if (filePath == null) return ServerResponse.error('Missing "file" (path to .appshots)');
        ...
        if (imported == null) return ServerResponse.error('Failed to parse .appshots file');
      ...
      case LibraryAction.search:
        ...
        final query = body['query'] as String? ?? request.uri.queryParameters['query'] ?? '';
        final allDesigns = await _persistenceService.getAllDesigns();
        final filtered = allDesigns.where((d) =>
