...
extension _EditorRoutes on CommandServer {
  ...
  Future<Map<String, dynamic>> handleEditor(
    ...
    final result = await _handleEditorAction(editorAction, method, request, cubit);
  ...
  Future<Map<String, dynamic>> _handleEditorAction(
    ...
    switch (editorAction) {
      ...
      case EditorAction.setBackground:
        ...
        if (colorStr == null) return ServerResponse.error('Missing "color" (hex string e.g. "#FF5733")');
        final color = parseHexColor(colorStr);
        if (color == null) return ServerResponse.error('Invalid color format: $colorStr');
      ...
      case EditorAction.setGradient:
        ...
        final gradient = ScreenshotDesign.fromJson({'backgroundGradient': gradientJson}).backgroundGradient;
      ...
      case EditorAction.setFrame:
        ...
        final device = Devices.all.where((d) =>
      ...
      case EditorAction.listFonts:
        ...
        if (query != null && query.isNotEmpty) {
          allFonts = allFonts.where((f) => f.toLowerCase().contains(query)).toList();
        }
      ...
      case EditorAction.setPadding:
        ...
        if (padding == null) return ServerResponse.error('Missing "padding" (number)');
      ...
      case EditorAction.setCornerRadius:
        ...
        if (radius == null) return ServerResponse.error('Missing "radius" (number)');
      ...
      case EditorAction.setOrientation:
        ...
        return ServerResponse.ok({'orientation': cubit.state.design.orientation.name});

      case EditorAction.setImage:
        final body = await _readBody(request);
        final filePath = body['file'] as String?;
        if (filePath == null) return ServerResponse.error('Missing "file" (path)');
        final file = File(filePath);
        if (!await file.exists()) return ServerResponse.error('File not found: $filePath');
      ...
      case EditorAction.setImageBase64:
        ...
        if (base64Data == null) return ServerResponse.error('Missing "data" (base64 PNG/JPG)');
        try {
          final bytes = base64Decode(base64Data);
          final tempDir = Directory.systemTemp;
          final fileName = 'cli_upload_${DateTime.now().millisecondsSinceEpoch}.png';
          final file = File('${tempDir.path}/$fileName');
          await file.writeAsBytes(bytes);
          cubit.updateImageFile(file);
          return ServerResponse.ok({'file': file.path, 'sizeBytes': bytes.length});
        } catch (e) {
      ...
      case EditorAction.addText:
        ...
        final textAlign = alignStr == 'left' ? TextAlign.left
            : alignStr == 'right' ? TextAlign.right
            : TextAlign.center;
        ...
        final updated = TextOverlay(
          ...
          style: TextStyle(
            ...
            color: colorStr != null ? (parseHexColor(colorStr) ?? Colors.white) : Colors.white,
          ),
          position: Offset(x ?? newOverlay.position.dx, y ?? newOverlay.position.dy),
          googleFont: fontName,
      ...
      case EditorAction.updateText:
        ...
        if (id == null) return ServerResponse.error('Missing "id" (overlay ID)');
        final existing = cubit.state.design.overlays
            .where((o) => o.id == id).firstOrNull;
        if (existing == null) return ServerResponse.error('Overlay not found: $id');

        var updated = existing;
        if (body.containsKey('text')) updated = updated.copyWith(text: body['text'] as String);
        if (body.containsKey('fontSize')) {
          updated = updated.copyWith(
            style: updated.style.copyWith(fontSize: (body['fontSize'] as num).toDouble()),
          );
        }
        if (body.containsKey('color')) {
          final c = parseHexColor(body['color'] as String);
          if (c != null) updated = updated.copyWith(style: updated.style.copyWith(color: c));
        }
        if (body.containsKey('font')) updated = updated.copyWith(googleFont: body['font'] as String);
        if (body.containsKey('x') || body.containsKey('y')) {
          updated = updated.copyWith(position: Offset(
        ...
        if (body.containsKey('rotation')) {
          updated = updated.copyWith(rotation: (body['rotation'] as num).toDouble());
        }
      ...
      case EditorAction.addImage:
        ...
        if (!success) return ServerResponse.error('Image overlay limit reached (max 10)');
        final overlay = cubit.state.design.imageOverlays.last;
        if (body.containsKey('x') || body.containsKey('y') ||
            body.containsKey('width') || body.containsKey('height')) {
          cubit.updateImageOverlay(overlay.id, overlay.copyWith(
            position: Offset(
      ...
      case EditorAction.updateImage:
        ...
        final existing = cubit.state.design.imageOverlays
            .where((o) => o.id == id).firstOrNull;
        if (existing == null) return ServerResponse.error('Image overlay not found: $id');
        var updated = existing;
        if (body.containsKey('x') || body.containsKey('y')) {
          updated = updated.copyWith(position: Offset(
            (body['x'] as num?)?.toDouble() ?? updated.position.dx,
        ...
        if (body.containsKey('width')) updated = updated.copyWith(width: (body['width'] as num).toDouble());
        if (body.containsKey('height')) updated = updated.copyWith(height: (body['height'] as num).toDouble());
        if (body.containsKey('scale')) updated = updated.copyWith(scale: (body['scale'] as num).toDouble());
        if (body.containsKey('rotation')) updated = updated.copyWith(rotation: (body['rotation'] as num).toDouble());
        if (body.containsKey('opacity')) updated = updated.copyWith(opacity: (body['opacity'] as num).toDouble());
        if (body.containsKey('cornerRadius')) updated = updated.copyWith(cornerRadius: (body['cornerRadius'] as num).toDouble());
      ...
      case EditorAction.setDisplayType:
        ...
        if (displayType == null) return ServerResponse.error('Missing "displayType" (e.g. APP_IPHONE_69)');
      ...
      case EditorAction.listOverlays:
        return ServerResponse.ok({
          'textOverlays': cubit.state.design.overlays.map((o) => {
            'id': o.id,
            'text': o.text,
          ...
          'imageOverlays': cubit.state.design.imageOverlays.map((o) => {
            'id': o.id,
            'filePath': o.filePath,
            'position': {'x': o.position.dx, 'y': o.position.dy},
            'width': o.width,
          ...
          'iconOverlays': cubit.state.design.iconOverlays.map((o) => {
            'id': o.id,
          ...
          'magnifierOverlays': cubit.state.design.magnifierOverlays.map((o) => {
            'id': o.id,
            'position': {'x': o.position.dx, 'y': o.position.dy},
            'width': o.width,
            'height': o.height,
      ...
      case EditorAction.setMeshGradient:
        ...
        final mesh = MeshGradientSettings.fromJson(body['mesh'] as Map<String, dynamic>);
        cubit.updateMeshGradient(mesh);
        return ServerResponse.ok({'meshGradient': 'set', 'pointCount': mesh.points.length});
      ...
      case EditorAction.updateIcon:
        ...
        final existing = cubit.state.design.iconOverlays
            .where((o) => o.id == id).firstOrNull;
        if (existing == null) return ServerResponse.error('Icon overlay not found: $id');
        var updated = existing;
        if (body.containsKey('x') || body.containsKey('y')) {
          updated = updated.copyWith(position: Offset(
            (body['x'] as num?)?.toDouble() ?? updated.position.dx,
        ...
        if (body.containsKey('size')) updated = updated.copyWith(size: (body['size'] as num).toDouble());
        if (body.containsKey('color')) {
          final c = parseHexColor(body['color'] as String);
      ...
      case EditorAction.updateMagnifier:
        ...
        final existing = cubit.state.design.magnifierOverlays
            .where((o) => o.id == id).firstOrNull;
        if (existing == null) return ServerResponse.error('Magnifier overlay not found: $id');
        var updated = existing;
        if (body.containsKey('x') || body.containsKey('y')) {
          updated = updated.copyWith(position: Offset(
            (body['x'] as num?)?.toDouble() ?? updated.position.dx,
        ...
        if (body.containsKey('width')) updated = updated.copyWith(width: (body['width'] as num).toDouble());
        if (body.containsKey('height')) updated = updated.copyWith(height: (body['height'] as num).toDouble());
        if (body.containsKey('zoomLevel')) updated = updated.copyWith(zoomLevel: (body['zoomLevel'] as num).toDouble());
        if (body.containsKey('cornerRadius')) updated = updated.copyWith(cornerRadius: (body['cornerRadius'] as num).toDouble());
      ...
      case EditorAction.saveDesign:
        final body = method == 'POST' ? await _readBody(request) : <String, dynamic>{};
        final name = body['name'] as String? ?? 'CLI Design ${DateTime.now().toIso8601String().substring(0, 16)}';
      ...
      case EditorAction.loadDesign:
        final body = await _readBody(request);
        final id = body['id'] as String?;
        if (id == null) return ServerResponse.error('Missing "id" (saved design ID)');
        final savedDesign = await _persistenceService.getDesignById(id);
        if (savedDesign == null) return ServerResponse.error('Design not found: $id');
        cubit.loadDesignIntoEditor(savedDesign);
        return ServerResponse.ok({'id': id, 'name': savedDesign.name});

      case EditorAction.export_:
        if (_captureCallback == null) {
          return ServerResponse.error('No capture callback registered. Is the editor UI visible?');
        }
        final body = method == 'POST' ? await _readBody(request) : <String, dynamic>{};
        _syncCallback?.call();
        await Future.delayed(const Duration(milliseconds: 100));
        ...
        if (bytes == null) {
          return ServerResponse.error('Capture failed — no image data returned');
        }
        var outputPath = body['path'] as String?;
        if (outputPath == null || outputPath.isEmpty) {
          final tempDir = Directory.systemTemp;
          outputPath = '${tempDir.path}/appshots_export_${DateTime.now().millisecondsSinceEpoch}.png';
        }
      ...
      case EditorAction.exportAll:
        if (_captureCallback == null || _multiCubit == null) {
          return ServerResponse.error('Capture callback or multi cubit not registered');
        }
        final body = method == 'POST' ? await _readBody(request) : <String, dynamic>{};
        ...
        final exportDir = outputDir ?? '${Directory.systemTemp.path}/appshots_export_${DateTime.now().millisecondsSinceEpoch}';
        ...
        for (int i = 0; i < designCount; i++) {
          ...
          editorCubit.loadDesignForMultiMode(
            ...
            imageFile: i < multiCubit.state.imageFiles.length ? multiCubit.state.imageFiles[i] : null,
          );
          ...
          if (imgBytes != null) {
            ...
            results.add({'index': i, 'path': filePath, 'sizeBytes': imgBytes.length});
          } else {
        ...
        editorCubit.loadDesignForMultiMode(
          ...
          imageFile: originalIndex < multiCubit.state.imageFiles.length ? multiCubit.state.imageFiles[originalIndex] : null,
        );
